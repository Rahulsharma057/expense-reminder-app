...
// Sends a push notification to every device a user has subscribed
// from. Best-effort: failures for one device never throw, and a
// subscription that the browser has revoked (410/404) is deleted so
  ...
  await Promise.all(
    subscriptions.map(async (sub) => {
      try {
        ...
        // 404/410 = the browser/OS says this subscription is dead.
...
module.exports = { sendPushToUser, sendPushToUsers, isPushConfigured: () => configured };